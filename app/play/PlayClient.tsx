    ...
    return (
      <main className="min-h-screen pb-20">
        ...
        <section className="max-w-md mx-auto px-6 -mt-8 relative z-10">
          ...
          {/* Share */}
          {(() => {
            // Build a shareable URL that carries the result so the OGP card
            // (generated by app/api/og/route.tsx) reflects this specific diagnosis.
            const shareParams = new URLSearchParams({
              result: resolvedEndpoint.resultLabel,
              desc: resolvedEndpoint.resultDescription,
            });
            const firstGenreIcon = GENRE_MAP[resolvedEndpoint.genreIds[0]]?.icon;
            if (firstGenreIcon) shareParams.set("icon", firstGenreIcon);
            const origin =
